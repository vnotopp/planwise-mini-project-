...
const VENUE_TYPES = [
  ...
  { value: 'banquet', label: 'Rented Banquet Hall' },
  ...
  return (
    <GlassCard delay={0.1} className="border-primary/20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        <div>
          ...
          <Select value={eventType} onValueChange={setEventType}>
            <SelectTrigger className="bg-muted/30 border-border mt-1 rounded-md"><SelectValue placeholder="Select type" /></SelectTrigger>
        ...
        <div>
          ...
          <Input type="number" value={guests} onChange={(e) => setGuests(e.target.value)} placeholder="e.g. 150" className="bg-muted/30 border-border mt-1 rounded-md" />
        </div>
        <div>
          <Label className="label-caps text-muted-foreground">Budget (INR)</Label>
          <Input type="number" value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="e.g. 500000" className="bg-muted/30 border-border mt-1 rounded-md" />
        </div>
        <div>
          <Label className="label-caps text-muted-foreground">Location</Label>
          <Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Mumbai" className="bg-muted/30 border-border mt-1 rounded-md" />
        </div>
        <div>
          <Label className="label-caps text-muted-foreground">Formality Level</Label>
          <Select value={formality} onValueChange={setFormality}>
            <SelectTrigger className="bg-muted/30 border-border mt-1 rounded-md"><SelectValue placeholder="Select formality" /></SelectTrigger>
            <SelectContent className="bg-card border-border">
        ...
        <div>
          ...
          <Select value={venueType} onValueChange={setVenueType}>
            <SelectTrigger className="bg-muted/30 border-border mt-1 rounded-md"><SelectValue placeholder="Select venue type" /></SelectTrigger>
            <SelectContent className="bg-card border-border">
        ...
        <div className="sm:col-span-2">
          ...
          <Input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className="bg-muted/30 border-border mt-1 rounded-md" />
        </div>
      </div>
      ...
      {!loading && suggestions.length > 0 && (
        <div className="mt-5 space-y-3">
          {suggestions.map((s, i) => (
            <div key={i} className="rounded-lg bg-muted/20 border border-border p-4">
